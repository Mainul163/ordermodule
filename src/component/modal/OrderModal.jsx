...
const OrderModal = ({ visible, onCreate, onCancel, foodServiceListData }) => {
	...
	const onSelect = (e) => {
		...
		// 	foodServiceListData?.foodService?.foodService?.vendorFoodAdditionalItems?.find(
		// 		(data) => data.id == e
		// 	);
		...
		<>
			<Modal
				...
				onOk={() => {
					form
						...
						.then((values) => {
							form.resetFields();
							onCreate(values, addItemQuantity.length);
						})
						.catch((info) => {
			...
			>
				...
				>
					...
					{visibleItem && (
						<Form.Item>
							<Button type="primary" onClick={() => addItems('hi')}>
								ADD food quantity
							</Button>
						</Form.Item>
					)}
